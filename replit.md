...
16. **Language / Data Source Cleanup + Turn-Based Panel Replacement** (Build 1.10.11) — (a) Completed full purge of all legacy "items", "workflow items", "overdue items", "critical items", "stage congestion" language from the Control Tower dashboard; (b) FULL PANEL REPLACEMENT: removed TrendSignalStrip (Completion Activity/Aging Items/Overdue Items/Stage Congestion pills), Asset Health Pulse card, WorkflowSpotlight (Active workflows ranked by concern), and BottleneckPanel (workflow-based); replaced with TurnInsightStrip (4 turn-based pills: Turn Completion/Blocked Turns/Not Rent-Ready/Rework Loop all sourced from turnStats), TurnBottleneckPanel (primary turn stage from turnStats.primaryBottleneckStage — shows blocked count, blocked %, avg completion, impact text, recommendation), TurnAgingPanel (3-row backlog: Blocked ≥7d, In Rework Loop, Not Rent-Ready — each with count/% severity, completion progress bar); removed dead code (fmtCost, portfolioExpiredCost/ExpiringSoonCost/TotalAssetCost variables, concernBorder utility, usePortfolio import); added RefreshCw + Home lucide icons; Driven By fallback block: "critical items" → "critical turns", "overdue items" → "overdue turns"; recommended action strings: all 4 metrics use turn terminology (in-progress turns, turn stage congestion, blocked turns, etc.); FlowReveal: "Workflow Congestion Signals" → "Turn Stage Congestion Signals", "Top Workflow Bottleneck" → "Top Turn Stage Bottleneck", "items stuck" → "turns stuck", "Affected Workflows" → "Bottleneck Stage Impact", "{n} open" → "{n} open turns"; RiskReveal: "Workflow Risk Signals" → "Turn Risk Signals", "Critical Items" card replaced with "Blocked Turns" (from turnStats.blockedTurns), "Overdue Items" card replaced with "Not Rent-Ready Units" (from turnStats.notRentReadyCount), "At-Risk Workflows" → "At-Risk Properties"; ExecutionReveal: "Workflow Execution Signals" → "Turn Execution Signals", "Turn Completion Rate" label, "Longest stuck item" → "Longest stalled turn", "stale workflow items" → "stalled turns", "stale workflows" → "stalled stages", "with open items" → "with open turns"; ImprovementReveal: "Workflow Improvement Signals" → "Turn Improvement Signals", "Oldest unresolved item" → "Oldest unresolved turn"; Stage Distribution chart: "Open items per stage" → "Open turns per stage"
